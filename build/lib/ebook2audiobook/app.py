...
# Construct the full path to default.wav
default_wav_path = os.path.join(script_dir, "default_voice.wav")

# Get the user's home directory path
home_directory = os.path.expanduser("~")
ebook2audiobook_files_dir = os.path.join(home_directory, "ebook2audiobook_files")
if not os.path.exists(ebook2audiobook_files_dir):
    os.makedirs(ebook2audiobook_files_dir)

# Check if default_voice.wav exists in the same directory as app.py
if not os.path.exists(default_wav_path):
    sys.exit("Error: 'default_voice.wav' file not found in the expected directory. Please ensure it is present.")
    ...
    try:
        ...
        with tqdm(unit='B', unit_scale=True, miniters=1, desc="Downloading Model") as t:
            def reporthook(blocknum, blocksize, totalsize):
                if t.total is None and totalsize:
                    t.total = totalsize
...
def create_chapter_labeled_book(ebook_file_path):
    ...
    ensure_directory(os.path.join(ebook2audiobook_files_dir, 'Working_files', 'Book'))
    ...
    def save_chapters_as_text(epub_path):
        ...
        directory = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp_ebook")
    ...
    # Example usage
    # input_ebook = 'path_to_ebook_file'
    # output_epub = 'output_epub_path'

    input_ebook = ebook_file_path  # Replace with your eBook file path
    output_epub = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp.epub")
    ...
    # Example usage
    folder_path = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp_ebook")
    output_csv = os.path.join(ebook2audiobook_files_dir, "Working_files", "Book", "Other_book.csv")
    ...
    # Paths
    input_folder = os.path.join(ebook2audiobook_files_dir, 'Working_files', 'temp_ebook')
    output_file = os.path.join(ebook2audiobook_files_dir, 'Working_files', 'Book', 'Chapter_Book.txt')
...
def convert_chapters_to_audio_custom_model(chapters_dir, output_audio_dir, temperature, length_penalty, repetition_penalty, top_k, top_p, speed, enable_text_splitting, target_voice_path=None, language=None, custom_model=None):
    ...
    if target_voice_path is None:
        target_voice_path = default_wav_path  # Ensure this is a valid path
    ...
    for chapter_file in sorted(os.listdir(chapters_dir)):
        if chapter_file.endswith('.txt'):
            ...
            temp_audio_directory = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp")
                ...
                for sentence in tqdm(sentences, desc=f"Chapter {chapter_num}"):
                    ...
                    for fragment in fragments:
                        if fragment != "":
                            ...
                            if custom_model:
                                # length penalty will not apply for custom models
                                ...
                                torch.save(out["wav"], fragment_file_path)  # Save the tensor as WAV
                            else:
                                speaker_wav_path = target_voice_path if target_voice_path else default_wav_path
...
def convert_ebook_to_audio(ebook_file, target_voice_file, language, use_custom_model, custom_model_file, custom_config_file, custom_vocab_file, temperature, length_penalty, repetition_penalty, top_k, top_p, speed, enable_text_splitting, custom_model_url=None, progress=gr.Progress()):
    ebook_file_path = ebook_file if ebook_file else None
    target_voice = target_voice_file if target_voice_file else None
    custom_model = None

    working_files = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp_ebook")
    full_folder_working_files = os.path.join(ebook2audiobook_files_dir, "Working_files")
    chapters_directory = os.path.join(ebook2audiobook_files_dir, "Working_files", "temp_ebook")
    output_audio_directory = os.path.join(ebook2audiobook_files_dir, 'Chapter_wav_files')
    ...
    if custom_model_url:
        ...
        download_dir = os.path.join(ebook2audiobook_files_dir, "Working_files", "custom_model")
    ...
    audiobook_output_path = os.path.join(ebook2audiobook_files_dir, "Audiobooks")
...
def run_gradio_interface():
    ...
    with gr.Blocks(theme=theme) as demo:
        ...
        output = gr.Textbox(label="Conversion Status (look at terminal for better approximation)")
    ...
    # Get the correct local IP or use localhost
    hostname = "localhost"
    local_ip = "127.0.0.1"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: ebook2audiobook --headless --ebook path_to_ebook --voice path_to_voice --language en --use_custom_model True --custom_model model.pth --custom_config config.json --custom_vocab vocab.json"
    )
    ...
    parser.add_argument("--language", type=str, default="en", 
                        help=f"Language for the audiobook conversion. Options: {', '.join(language_options)}. Defaults to English (en).")
    ...
    parser.add_argument("--speed", type=float, default=1.0, help="Speed factor for the speech generation. IE: How fast the Narrator will speak. Defaults to 1.0.")
    ...
    if args.headless:
        # If the arg.custom_model_url exists then use it as the custom_model_url
